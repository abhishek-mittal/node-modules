...
export const NavbarClasses = (props, colors, classes) => {
  ...
  return {
    base: {
      common: cls(`w-full z-20 top-0 pt-safe`, positionClass('sticky', classes)),
      ios: fontSizeIos,
      material: fontSizeMaterial
    },
    bg: {
      common: cls('absolute w-full h-full left-0 top-0', bgClassName || bgClass),
      ios: cls(colors.bgIos, hairlines && 'hairline-b', translucent && 'translucent'),
      material: `shadow-md ${colors.bgMaterial}`
    },
    subnavbar: {
      common: cls('relative flex items-center', subnavbarClassName || subnavbarClass),
      ios: 'h-11 pl-2-safe pr-2-safe',
      material: 'h-12 pl-4-safe pr-4-safe'
    },
    inner: {
      common: cls('flex relative items-center w-full overflow-hidden', innerClassName || innerClass),
    ...
    title: {
      common: cls(`${colors.title} whitespace-nowrap leading-tight`, titleClassName || titleClass),
